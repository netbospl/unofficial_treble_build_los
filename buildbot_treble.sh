#!/bin/bash
echo ""
echo "Uofficial /e/ OS v1-q branch Treble Buildbot"
...
echo "Cleanup/reset repos before sync"
repo forall -vc "git reset --hard; git am --abort; git revert --abort; git am --abort" &> /dev/null
sleep 5
echo ""
...
buildVariant() {
	...
	mv $OUT/system.img ~/build-output/eos-q-$BUILD_DATE-UNOFFICIAL-${1}.img
}
